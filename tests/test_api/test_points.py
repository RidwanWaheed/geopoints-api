...
def test_nearby_points(client, test_points):
    ...
    response_large = client.get(f"/api/v1/points/nearby?lat={lat}&lng={lng}&radius=5000")
...
def test_nearest_points(client, test_points):
    ...
    response = client.get(
        f"/api/v1/points/nearest?lat={lat}&lng={lng}&limit=3"
    )
...
def test_within_polygon(client, test_points):
    ...
    # Make API request
    response = client.post(
        f"/api/v1/points/within?polygon={polygon_wkt}"
    )
