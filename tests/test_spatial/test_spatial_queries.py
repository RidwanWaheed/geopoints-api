...
def filter_by_distance(
    ...
    return query.having(
        func.ST_Distance(
            func.geography(model_class.geometry), func.geography(point_geom)
        )
        <= radius
    )
...
def test_nearest_neighbor_query(db_session, test_points):
    ...
    assert (
        abs(nearest_distance - haversine_distance) < 10
    ), "Distance calculation is significantly different from Haversine"
...
def test_points_within_polygon(db_session, test_points):
    ...
    # Convert to WKT
    polygon_wkt = (
        f"POLYGON(({', '.join([f'{x} {y}' for x, y in polygon.exterior.coords])}))"
    )

    # Use raw SQL with ST_Within to find points in the polygon
    query = db_session.query(Point).filter(
        func.ST_Within(
            Point.geometry, func.ST_GeomFromText(polygon_wkt, SpatialRefSys.WGS84)
        )
    )
