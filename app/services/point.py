...
class PointService:
    ...
    def create(self, *, obj_in: PointCreate) -> PointSchema:
        """Create a new point"""
        db_obj = self.repository.create_with_coords(obj_in=obj_in)
    ...
    def update(self, *, id: int, obj_in: PointUpdate) -> PointSchema:
        """Update a point"""
        db_obj = self.repository.get(id=id)
        if not db_obj:
            return None
    ...
    def remove(self, *, id: int) -> PointSchema:
        """Remove a point"""
        db_obj = self.repository.remove(id=id)
