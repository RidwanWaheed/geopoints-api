...
def upgrade():
    ...
    op.create_index('idx_points_geography', 'points', [sa.text("geography(geometry)")], unique=False, postgresql_using='gist')
    ...
    op.execute("CREATE INDEX idx_points_geometry_knn ON points USING GIST (geometry gist_geometry_ops_nd)")
...
def downgrade():
    # Remove new indexes
    op.drop_index('idx_points_geometry_knn', table_name='points', postgresql_using='gist')
    ...
    op.create_index('idx_point_geometry', 'points', ['geometry'], unique=False, postgresql_using='gist')
